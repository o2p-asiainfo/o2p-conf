...
function queryExistsAuthenticate(){
	$("#form1").submit();
}
...
<script>
	...
	function deleteProof(value){
		if (confirm("<s:property value="%{getText('eaap-op2-conf-authenticate.deleteConfirm')}"/>")) {
			$.ajax({
				type:'post',
				url:'deleteProofEffect.shtml',
				data:'proofEffect.proofeId='+value,
				dataType:'text',
				success:function(msg){
				    $("#"+value).remove();
					alert(msg);				
...
<div class="contentWarp">
      ...
      <s:property value="%{getText('eaap-op2-conf-authenticate.authenticate')}"/>
       <img src="../resource/blue/images/module-path.png" />
      <s:property value="%{getText('eaap-op2-conf-authenticate.authenticateSelect')}"/>
      </div>
    </div>
    ...
    <div>
       <form  name="myForm" action="../authenticate/queryExistsAuthenticate.shtml" method="post" id="form1"> 
			...
			<tr>
	   		...
	   		<tr>
	   			<td colspan=4 style="TEXT-ALIGN: right;PADDING-RIGHT:30px; ">
  					<a class="button-base button-small" title="<s:property value="%{getText('eaap.op2.conf.techimpl.queryBtn')}" />" target="_blank" onclick="queryExistsAuthenticate()">
...
<script type="text/javascript">
	 ...
	 function do_qry(){
	 	...
	 	$("#list_iframe").attr("src","../authenticate/queryExistsAuthenticate.shtml");
	 	$('#form1').submit();
	 }
