...
<html>
  ...
  <script type="text/javascript">
	 ...
	 function drawLine(){
		...
		var chart = new FusionCharts("../resource/comm/swf/MSLine.swf", "chart_"+<%=componentId %>, "100%", 260, "0", "1");
	 	$.ajax({
			url : '<%=request.getContextPath()%>/monitorView/loadComponentViewData.shtml',
			cache : false,
			async : false,
			dataType : "json",
			contentType:'application/x-www-form-urlencoded;charset=UTF-8',
			type  : 'POST',
			data  : {tabType:<%=tabType%>,componentId:<%=componentId%>,sysErrTotal:sysErrTotal,bizTotal:bizTotal,bizErrTotal:bizErrTotal,yUnits:yUnits,allPerfomance:allPerfomance,csbPerfomance:csbPerfomance,supplyPerfomance:supplyPerfomance},
			success : function(response) {
	 	 		chart.setDataXML(response[0]); 
			}
		});
    	chart.render("<%=componentId %>");
	 }
