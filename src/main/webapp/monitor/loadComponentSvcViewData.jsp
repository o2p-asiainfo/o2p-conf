...
<html>
  ...
  <%
  		...
  		String componentSvcId = StringEscapeUtils.escapeHtml4(request.getParameter("componentSvcId"));
   %>
  <div id="<%=componentSvcId %>"></div>
  <script type="text/javascript">
	 ...
	 function drawLine(){
		...
		var chart = new FusionCharts("../resource/comm/swf/MSLine.swf", "compSvcChart", "100%", 260, "0", "1");
	 	$.ajax({
			url : '<%=request.getContextPath()%>/monitorView/loadComponentSvcViewData.shtml',
			cache : false,
			async : false,
			type : 'post',
			contentType:'application/x-www-form-urlencoded;charset=UTF-8',
			data : {componentSvcId :"<%=componentSvcId%>",componentId:"<%=componentId%>",tabType:"<%=tabType%>",sysErrTotal:sysErrTotal,bizTotal:bizTotal,bizErrTotal:bizErrTotal,yUnits:yUnits,allPerfomance:allPerfomance,csbPerfomance:csbPerfomance,supplyPerfomance:supplyPerfomance},
			dataType : "json",
			success : function(response) {
	 	 		chart.setDataXML(response[0]); 
			}
		});
    	chart.render("<%=componentSvcId %>");
	 }
</script>
</html>
