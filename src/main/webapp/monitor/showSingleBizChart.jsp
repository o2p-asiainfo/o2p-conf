...
<html>
  ...
  <body>
	  <div id="bizTab" class="easyui-tabs" style="height:300px">
  	 	...
  	 	$('#bizTab').tabs({
			...
			onSelect:function(title){
				 if(title == "<s:property value="%{getText('eaap.op2.conf.monitor.view.serviceUse')}"/>"){
				 	$("#svcUseLine").attr("src","../monitor/loadSingleBizData.jsp?tabType=2&bizCode="+<%=bizCode%>);
				 }else if(title == "<s:property value="%{getText('eaap.op2.conf.monitor.view.serviceReg')}"/>"){
				 	$("#svcRegLine").attr("src","../monitor/loadSingleBizData.jsp?tabType=3&bizCode="+<%=bizCode%>);
				 }else{
				 	$("#performance").attr("src","../monitor/loadSingleBizData.jsp?tabType=1&bizCode="+<%=bizCode%>);
				 }
			}
		});
