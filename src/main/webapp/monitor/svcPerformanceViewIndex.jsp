...
<html>
 ...
 <!--body start -->
  <body>
  <form method="post" name="myform" action="${contextPath}/monitorView/showServiceViewIndex.shtml">
		    				...
		    				<td width="300px" style="margin-top: -2px;">
		    					<div>
				                  <div style="float:left">
				              	  ...
				              	  <div style="float:left;">
				             		<ui:select skin="${contextStyleTheme}" onchange="changeView(this.value);" name="viewSwitch" value="3" id="viewSwitch" list="viewSwitchMaps" listKey="ID" listValue="NAME" width="160" layerWidth="165" layerHeight="100"></ui:select>
				                  </div>
				              </div>
		...
		<div>
	    	<table class="mgr-table" style="font-size:12px;">
	    		<tr>
	    			...
	    			<td class="item"><s:property value="%{getText('eaap.op2.conf.monitor.view.serviceName')}" />:</td>
	    			<td>
	   					<ui:multiSelectBox skin="${contextStyleTheme}" id="svc_code_box" name="service.serviceCode" list="serviceList" listKey="serviceCode" listValue="serviceCnName"></ui:multiSelectBox>
	    			</td>
   	   ...
   	   <ui:form name="portalform">
		   <div id="portal">
		     <c:choose>
		     	<c:when test="${fn:length(svcPerformanceList)<1}">
		     	...
		     	<c:otherwise>
		     		 <c:forEach items="${svcPerformanceList}" var="svcPerformance" varStatus="status" step="1" >
						<c:choose>
							<c:when test="${status.count%2==0}">
								 <div style="margin-top:5px;float:right;">
								 	<em id="tipTitle" title="${svcPerformance.ORG_NAME}-${svcPerformance.COMPONENT_NAME}-${svcPerformance.SERVICE_CN_NAME}"/>
								 	<div name="_comptPanel" id="${svcPerformance.SERVICE_ID}" class="easyui-panel" style="height:330px;" data-options="title:'${svcPerformance.SERVICE_CN_NAME}',collapsible:true,tools:[{iconCls:'icon-add',handler:function(){addPanel('${svcPerformance.COMPONENT_ID}','${svcPerformance.SERVICE_CODE}','${svcPerformance.SERVICE_CN_NAME}','${svcPerformance.ORG_NAME}-${svcPerformance.COMPONENT_NAME}-${svcPerformance.SERVICE_CN_NAME}');}}]">
										<iframe src="${contextPath}/monitor/showSvcPerformanceChart.jsp?serviceCode='${svcPerformance.SERVICE_CODE}'&componentId=${svcPerformance.COMPONENT_ID}" frameborder="0" scrolling="no" style="border:0;width:100%;height:300px;"></iframe>
									</div>
								</div>
							</c:when>
							<c:otherwise>
...
<script type="text/javascript">
	function addPanel(componentId,serviceCode,title,tipTitle){
		$.ajax({
			...
			type  : 'POST',
			data  : {type:4,componentId:componentId,serviceCode:serviceCode,title:title,tipTitle:tipTitle},
