...
<html>
  ...
  <script type="text/javascript">
	 ...
	 function drawLine(){
		...
		var chart = new FusionCharts("../resource/comm/swf/MSLine.swf", "bizChart", "100%", 260, "0", "1");
	 	$.ajax({
			url : '<%=request.getContextPath()%>/monitorView/querySingleBizData.shtml',
			cache : false,
			async : false,
			type : 'post',
			contentType:'application/x-www-form-urlencoded;charset=UTF-8',
			data : {bizCode :"<%=bizCode%>",tabType:"<%=tabType%>",sysErrTotal:sysErrTotal,bizTotal:bizTotal,bizErrTotal:bizErrTotal,yUnits:yUnits,allPerfomance:allPerfomance,csbPerfomance:csbPerfomance,supplyPerfomance:supplyPerfomance},
			dataType : "json",
			success : function(response) {
	 	 		chart.setDataXML(response[0]); 
			}
		});
    	chart.render("<%=bizCode %>");
	 }
