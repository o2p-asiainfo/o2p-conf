...
<html>
  ...
  <body>
	  <div id="componentSvcTab" class="easyui-tabs" style="height:300px">
  ...
  <%
  	  String serviceId = StringEscapeUtils.escapeHtml4(request.getParameter("svcId"));
  ...
  <script type="text/javascript">
	...
	function changeTab(){
		$('#componentSvcTab').tabs({
			cache:false,
			tabPosition:'bottom',
			onSelect:function(title){
				 if(title == "<s:property value="%{getText('eaap.op2.conf.monitor.view.serviceUse')}"/>"){
				 	$("#svcUseLine").attr("src","../monitor/loadComponentSvcViewData.jsp?tabType=2&componentId="+<%=componentId%>+"&componentSvcId="+<%=serviceId%>);
				 }else if(title == "<s:property value="%{getText('eaap.op2.conf.monitor.view.serviceReg')}"/>"){
				 	$("#svcRegLine").attr("src","../monitor/loadComponentSvcViewData.jsp?tabType=3&componentId="+<%=componentId%>+"&componentSvcId="+<%=serviceId%>);
				 }else{
				 	$("#performance").attr("src","../monitor/loadComponentSvcViewData.jsp?tabType=1&componentId="+<%=componentId%>+"&componentSvcId="+<%=serviceId%>);
				 }
			}
		});
