...
dhtmlXTreeObject.prototype._doContClick=function(ev){
    ...
    this.callEvent("onRightClick",[obj.id,ev]);
...
*     @topic: 0
*/
	dhtmlXTreeObject.prototype.loadXML=function(file,afterCall,param){ 
      ...
      if (afterCall) this.XMLLoader.waitCall=afterCall;
      this.XMLLoader.loadXML(file,'post',param);
   };
/**
...
//      itemObject.span.appendChild(document.createTextNode(itemObject.label));
      ...
      var temp=this;
      if(td2.attachEvent){
      	//td2.attachEvent('onclick', temp.prototype.onRowClick2);
	  }else{
      	//td2.addEventListener("click", this.onRowClick2, false);
      }
...
*     @topic: 0  
*/
   dhtmlXTreeObject.prototype.setXMLAutoLoading=function(filePath,param){  this.XMLsource=filePath;this.autoLoadParam=param; };
...
*/
   dhtmlXTreeObject.prototype.getUserData=function(itemId,name){
      ...
      return sNode.userData["t_"+name]!=undefined?(sNode.userData["t_"+name].replace(/tempsurl;/g,"&")):(sNode.userData["t_"+name]);
   };
...
*/
   dhtmlXTreeObject.prototype._loadDynXML=function(id,src) {
   		...
   		param = this.autoLoadParam;
        var sn=(new Date()).valueOf();
        this._ld_id=id;
			if(this.getItemText(id).toUpperCase().indexOf('CHECKED')!=-1){
            	this.loadXML(src+getUrlSymbol(src)+"uid="+sn+"&id="+this._escape(id)+"&tocancelcheckbox=true",null,param);
            }else if(this.getItemText(id).toUpperCase().match(/DISABLED/g)!=null&&this.getItemText(id).toUpperCase().match(/DISABLED/g).length>=1){
            	this.loadXML(src+getUrlSymbol(src)+"uid="+sn+"&id="+this._escape(id)+"&tocancelcheckbox=true",null,param);
            }else{
            	this.loadXML(src+getUrlSymbol(src)+"uid="+sn+"&id="+this._escape(id),null,param);
            }
        };
