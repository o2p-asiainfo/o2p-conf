		...
		jQuery.extend(jQuery.fn,{
			uploadify:function(options) {
				jQuery(this).each(function(){
					...
					width          : 140, // The width of the flash button
					...
					onProgress     : function() {}, // 工作进度 Function to run each time the upload progress is updated
					onComplete     : function() {}, // 完备 Function to run when an upload is completed
					onAllComplete  : function() {}  //在全部完成 Functino to run when all uploads are completed
				}, options);
				...
				jQuery(this).bind("uploadifySelect", {'action': settings.onSelect, 'queueID': settings.queueID}, function(event, ID, fileObj) {
					if (event.data.action(event, ID, fileObj) !== false) {
						...
						if (event.data.queueID) {							
							if(event.data.queueID=='imageQueue'){//图片
								addImgUI_img(ID);//创建图片框
							    queue = '#' + event.data.queueID +'_'+ ID;
							}else if(event.data.queueID=='fileQueue'){
								addFileUI(ID);
							    queue = '#' + event.data.queueID;
							}else if(event.data.queueID=='imageQueueOther'){//同一个jsp引用多个组件
								addImgUI_imgOther(ID);//创建图片框
							    queue = '#' + event.data.queueID +'_'+ ID;
							}else {
								addOtherUI(ID);
							    queue = '#' + event.data.queueID;
						...
						var attStr = 'attrPicQueue';
						var strstr = event.data.queueID.substring(0,attStr.length);
						//如下为进度条的控制						
					    if(event.data.queueID && event.data.queueID=='imageQueue'){					   
					    	 //图片
					        jQuery(queue).append('<div id="' + jQuery(this).attr('id') + ID + '" class="uploadifyQueueItem_img"> <span class="percentage"></span><div class="uploadifyProgress"><div id="' + jQuery(this).attr('id') + ID + 'ProgressBar" class="uploadifyProgressBar_img"></div></div></div>');
					    }else if(event.data.queueID && event.data.queueID=='fileQueue'){
					        jQuery(queue).append('<div id="' + jQuery(this).attr('id') + ID + '" class="uploadifyQueueItem"><div class="cancel"><a href="javascript:jQuery(\'#' + jQuery(this).attr('id') + '\').uploadifyCancel(\'' + ID + '\')" onclick="if(window.event){window.event.returnValue = false;}"><img src="' + settings.cancelImg + '" border="0"  /></a></div><span class="fileName">' + fileName + ' (' + byteSize + suffix + ')</span><span class="percentage"></span><div class="uploadifyProgress"><div id="' + jQuery(this).attr('id') + ID + 'ProgressBar" class="uploadifyProgressBar"></div></div></div>');				    					   
					    }else{
					      //其它
					       jQuery(queue).append('<div id="' + jQuery(this).attr('id') + ID + '" class="uploadifyQueueItem"><div class="cancel"><a href="javascript:jQuery(\'#' + jQuery(this).attr('id') + '\').uploadifyCancel(\'' + ID + '\')" onclick="if(window.event){window.event.returnValue = false;}"><img src="' + settings.cancelImg + '" border="0"  /></a></div><span class="fileName">' + fileName + ' (' + byteSize + suffix + ')</span><span class="percentage"></span><div class="uploadifyProgress"><div id="' + jQuery(this).attr('id') + ID + 'ProgressBar" class="uploadifyProgressBar"></div></div></div>');				    					   
					    }
				...
				jQuery(this).bind("uploadifyQueueFull", {'action': settings.onQueueFull}, function(event, queueSizeLimit) {
					if (event.data.action(event, queueSizeLimit) !== false) {
						alert('您最多一次可选择 ' + queueSizeLimit + ' 张图片上传。')
					}
				});
				...
				jQuery(this).bind("uploadifyComplete", {'action': settings.onComplete}, function(event, ID, fileObj, response, data) {
					if (event.data.action(event, ID, fileObj, unescape(response), data) !== false) {
						...
						jQuery("#" + jQuery(this).attr('id') + ID).fadeOut(1, function() { jQuery(this).remove()});
					}
				});
