...
function initUploadify(){
	jQuery("#uploadify").uploadify({
		...
		'script'					: '../fileShare/uploadFile.shtml;jsessionid='+$("#jsessionid").val(),//处理上传的路径，这里使用Struts2是XXX.action
		'scriptData':{'USER_TENANT_ID':$("#USER_TENANT_ID").val()},
		...
		'width'			 : 90,
		'height'		 : 30,
		'fileExt'		 :  '*.rar;*.zip;*.wsdl;*.xsd',	// 设置可以选择的文件的类型，格式如：'*.rar;*.zip;*.wsdl;*.xsd' 。
		'fileDesc'		 : 'choose *.rar/*.zip/*.wsdl/*.xsd',// 设置文件浏览对话框中的文件类型下拉框的显示文本。
		'onInit'         : function() { 
							},
