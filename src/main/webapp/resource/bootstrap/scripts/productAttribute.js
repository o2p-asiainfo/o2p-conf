        ...
        $("#chooseContract").click(function(){
            ...
            if (oSelected.size() == 1) {
            	...
            	if("L" == action){
                	...
                	//加载右节点
                	getNodesc(selected.tcpcrtfid,"R",selected.contypeNum,selected.name);
...
function saveContractAdapter(){
	...
	var pageSrcTcpCtrFId = $('#pageSrcTcpCtrFId').val();//源节点
		$.ajax({
		    ...
		    dataType:'json',
		    data:{pageSrcTcpCtrFId:pageSrcTcpCtrFId,pageTarTcpCtrFId:pageTarTcpCtrFId,type:'0',svname:$('#srcContractVersionName').val(),tvname:$('#tarContractVersionName').val(),pageContractAdapterId:$('#pageContractAdapterId').val(),pageEndpointId:$('#pageEndpointId').val()},
			success:function(msg){
