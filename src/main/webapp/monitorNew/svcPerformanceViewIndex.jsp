...
<div class="contentWarp"  style="min-height: 420px;">
   	<table>
   		<tr>
   			<td style="width:160px; text-align:center;"><s:property value="%{getText('eaap.op2.conf.monitor.view.serviceName')}" />:</td>
   			<td>
   					<select id="serviceCode" name="service.serviceCode"  class="js-example-basic-multiple" multiple="multiple"  style="width:250px;">
				     	<c:forEach items="${serviceList}" var="sService" varStatus="status" step="1" >
				     		<option value="${sService.serviceCode}">${sService.serviceCnName}</option>
						</c:forEach>
					</select>
   			</td>
...
<script type="text/javascript">
	...
	function setPageHtml(recordList){
		...
		if(recordList.length>0){
			for (var i=0; recordList.length>i; i++){
				var _serviceCode 	= recordList[i].SERVICE_CODE;
				var _componentId 	= recordList[i].COMPONENT_CODE;
				var _name				= recordList[i].SERVICE_CN_NAME;
				var _tipTitle				= recordList[i].ORG_NAME+"-"+recordList[i].COMPONENT_NAME+"-"+recordList[i].SERVICE_CN_NAME;
				var _position = "left";
				if(i%2==1){
					_position = "right";
				}
				retHtml +="<div style=\"margin-top:5px;float:"+_position+"; width:49.5%; height:358px;\">";
				retHtml +="		<iframe src=\"${contextPath}/monitorNew/showSvcPerformanceChart.jsp?serviceCode="+_serviceCode+"&componentId="+_componentId+"&panelTitle="+encodeURIComponent(encodeURIComponent(_name))+"&tipTitle="+encodeURIComponent(encodeURIComponent(_tipTitle))+"\" frameborder=\"0\" scrolling=\"no\" style=\"border:0;width:100%;height:358px\"></iframe>";
				retHtml +="</div>";
			}
		}else{
	...
	function getData(num){
		...
		$.ajax({
		     type:"post",
		     async:false,
		     url:"${contextPath}/monitorView/getServiceViewIndexNewList.shtml",
		     dataType:"text",
		     data:"currentPage="+num+"&serviceCode="+serviceCode,
