...
<div class="contentWarp"  style="min-height: 420px;">
   	<table >
   		<tr>
   			<td style="width:160px; text-align:center;"><s:property value="%{getText('eaap.op2.conf.monitor.view.bizFunctionName')}" />:</td>
   			<td>
   				<select id="bizCode" name="bizFunction.code"  class="js-example-basic-multiple" multiple="multiple"  style="width:250px">
				     	<c:forEach items="${bizFunctionList}" var="bizFunction" varStatus="status" step="1" >
				     		<option value="${bizFunction.code}">${bizFunction.name}</option>
						</c:forEach>
				</select>
   			</td>
...
<script type="text/javascript">
//addPanel('${singleBiz.CODE}','${singleBiz.NAME}','${singleBiz.ORG_NAME}-${singleBiz.COMPONENT_NAME}-${singleBiz.NAME}')
function addPanel(bizCode,title,tipTitle){
	...
	function setPageHtml(recordList){
		...
		if(recordList.length>0){
			for (var i=0; recordList.length>i; i++){
				var _code 	= recordList[i].CODE;
				var _name	= recordList[i].NAME;
				var _tipTitle= recordList[i].ORG_NAME+"-"+recordList[i].COMPONENT_NAME+"-"+recordList[i].NAME;
				var _position = "left";
				if(i%2==1){
					_position = "right";
				}
				retHtml +="<div style=\"margin-top:5px;float:"+_position+"; width:49.5%; height:358px;\">";
				retHtml +="		<iframe src=\"${contextPath}/monitorNew/showSingleBizChart.jsp?bizCode="+_code+"&panelTitle="+encodeURIComponent(encodeURIComponent(_name))+"&tipTitle="+encodeURIComponent(encodeURIComponent(_tipTitle))+"\" frameborder=\"0\" scrolling=\"no\" style=\"border:0;width:100%;height:358px\"></iframe>";
				retHtml +="</div>";
			}
		}else{
	...
	function getData(num){
		...
		$.ajax({
		     type:"post",
		     async:false,
		     url:"${contextPath}/monitorView/getSingleBizViewIndexList.shtml",
		     dataType:"text",
		     data:"currentPage="+num+"&bizCode="+bizCode,
