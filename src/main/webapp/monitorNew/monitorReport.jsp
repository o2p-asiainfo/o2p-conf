	    ...
	    <div>
			...
			<tr>
				<td colspan="6" style="TEXT-ALIGN:center; margin:0;padding:5;">
					...
					<a class="button-base button-small" title="<s:property value="%{getText('eaap.op2.conf.monitor.report.excelBtn')}"/>" target="_blank"  onclick="expExcel();">
...
<script type="text/javascript">
	function query(){
			if(validatorDate()&&validatorDimen() && validatorStat()){
				//mask();
	...
	function selectTab03Syn(i)
	{
		...
		function validatorDate(){
			if($("#dateType").val()=='1'){
				var start  = new Date($("#btime").val().replace(/-/g,"/")).getTime();
				var end = new Date($("#etime").val().replace(/-/g,"/")).getTime();
				var len=end - start;
				if(len> 31*24*60*60*1000 ){
					commShowAlert(document.getElementById("dateDiv1"),"<s:property value="%{getText('eaap.op2.conf.monitor.report.dateValidator')}"/>");
					return false;
				}else {
					return true;
				}
