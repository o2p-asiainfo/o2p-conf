...
<html>
	...
	<body>
		...
		<ui:form method="post" name="addDirForm" id="addDirForm"
			...
			<ui:inputText skin="${contextStyleTheme}" name="directory.dirId" id="dirId" type="hidden" value="${directory.dirId }"></ui:inputText>
			...
			<div class="contentWarp">
				<div>
					...
					<table class="mgr-table">
						<tr>
							...
							<td class="item-content">
								<ui:inputText skin="${contextStyleTheme}" type="text" name="directory.dirName"
									id="dirName"  value="${directory.dirName }"/>
 		        ...
 		        attrId="qweid" attrPid="qwepId"  attrName="qwepName" params="fId:${directory.faDirId}" cascadeCheck=""  attrIsParent="qweIsParent" saveIdTextName="faDirId" saveIdTextId="faDirId1" >
 		        </ui:seltree>
                </td>         
						</tr>
						<script>
						...
						<tr>
							...
							<td colspan="3" style="padding-left: 5px;">
			     				<input id="imgUrl" type="hidden"  name="directory.sFileId" value="${directory.SFileId}"/>
								...
								<br/>
								<c:choose>
					                <c:when test="${directory.SFileId!=null and directory.SFileId!=''}">
					                  <img id="imagecard" src="${contextPath}/fileShare/readImg.shtml?fileShare.sFileId=${directory.SFileId}" width="100" height="100"/>
					                </c:when>
					                <c:otherwise>
								...
								<td class="item-content" style="display:none">
						 			<input type="text" size="30" name="directory.dirType" 	value="${directory.dirType }"  id="dirType" style="display: none;"/> 
								</td>
						</tr>
						...
						<tr>
							...
							<td colspan="4" style="TEXT-ALIGN: center;">
								...
								<ui:button id="butSubmit" skin="${contextStyleTheme}" onclick="if(comm_validators_check('group1')){updateDir();}"
	...
	function ale()
	{
	 ...
	 function addServiceManager(){
		...
		function updateDir(){	 
			...
			var faId = $("#faDirId1").val();
			var dirId = $("#dirId").val();
			
			if(faId==dirId){
				
				alert("<s:property value="%{getText('eaap.op2.conf.directory.manager.parentIsSelf')}" />");
				return;
			}
			...
			$.ajax({
				type: "POST",
				async:false,
			    url: "${contextPath}/directoryManager/isChildDir.shtml",
			    dataType:'json',
			    data:{dirId:$("#faDirId1").val(),faDirId:$("#dirId").val()},
					success:function(msg){ 
						 if(JSON.stringify(msg)=='true'){
							 ...
							 $.ajax({
									type: "POST",
									async:false,
								    url: "${contextPath}/directoryManager/isExistDirName.shtml",
								    dataType:'json',
								    data:{dirId:$("#dirId").val(),dirName:$("#dirName").val(),faDirId:$("#faDirId1").val(),dirType:$("#dirType").val()},
									success:function(msg){ 
											 if(JSON.stringify(msg)=='true'){
												 $.ajax({
											            type:"post",
											            async:false,
											            url:"../directoryManager/updateDirectory.shtml",
											            dataType:"json",
											            data:{dirId:$("#dirId").val(),dirName:$("#dirName").val(),faDirId:$("#faDirId1").val(),sFileId:$("#imgUrl").val(),dirType:$("#dirType").val()},
											            success:function(msg){
